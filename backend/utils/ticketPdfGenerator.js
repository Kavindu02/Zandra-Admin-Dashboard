...
/**
 * Server-side PDF generator for flight e-tickets.
 */
const generateTicketPDFBuffer = async (customer) => {
  try {
    ...
    if (typeof returnSegments === 'string') {
        try { returnSegments = JSON.parse(returnSegments); } catch (e) { returnSegments = []; }
    }
    ...
    for (let i = 0; i < renderFlights.length; i++) {
      ...
      if (fl.equipment) {
        page.drawText(`Equipment: ${fl.equipment}`, { x: 38, y: currentY + 2, size: 8, font, color: rgb(0.35, 0.40, 0.48) });
      }
